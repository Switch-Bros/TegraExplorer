...
static u8 _hid_transfer_start(usb_ctxt_t *usbs, u32 len)
{
	...
	{
		usbs->set_text(usbs->label, "#FFDD00 Fehler:# EP IN uebertragung!");
...
int usb_device_gadget_hid(usb_ctxt_t *usbs)
{
	...
	usbs->set_text(usbs->label, "#C7EA46 Status:# USB gestartet");
	...
	usbs->set_text(usbs->label, "#C7EA46 Status:# Warte auf Verbindung");

	// Initialize Control Endpoint.
	if (usb_ops.usb_device_enumerate(gadget_type))
		goto error;

	usbs->set_text(usbs->label, "#C7EA46 Status:# Warte auf HID Reportanfrage");

	if (usb_ops.usb_device_class_send_hid_report())
		goto error;

	usbs->set_text(usbs->label, "#C7EA46 Status:# HID Emulation gestartet");
	...
	usbs->set_text(usbs->label, "#C7EA46 Status:# HID beendet");
	goto exit;
