		...
		if (*in == '#') {
			if (!memcmp(in + 1, "REQUIRE ", 8)) {
				if (!memcmp(in + 9, "VER ", 4)) {
					...
					if (outdated) {
						printScriptError(SCRIPT_LEXER_FATAL, "Das Skript benoetigt eine neuere TegraExplorer-Version!");
					...
					#endif
					if (!minervaEnabled) {
						printScriptError(SCRIPT_LEXER_FATAL, "Erweiterter Speicher erforderlich.\nKopiere den bootloader-Ordner von Hekate auf deine SD-Karte!");
						return Token_Fatal_Err;
					}
				}
					...
					#endif
					if (!gotSd){
						printScriptError(SCRIPT_LEXER_FATAL, "SD-Karte benoetigt.");
						return Token_Fatal_Err;
					}
				}
			}
...
ParserRet_t parseScript(char* in, u32 len) {
			...
			if (token == Equals && lastOp) {
				if (lastOp->token == Variable) {
					if (lastOp->variable.staticVariableSet) {
						SCRIPT_PARSER_ERR("Zuweisungsversuch an eine statische Variable");
					}
					else {
					...
					SCRIPT_PARSER_ERR("Zuweisungsversuch an ein Nicht-Objekt");
				}
			}
				...
				if (stackHistoryHolder.count != 1 && *lastHistory == History_Function) {
					...
					if (lastOp && (lastOp->token == Variable || (lastOp->token == CallArgs && !isLastVarSet(lastOp)))) {
						if (lastOp->token == Variable) {
							SCRIPT_PARSER_ERR("GET-Variable vor {}");
							continue;
						}
					...
					SCRIPT_PARSER_ERR("Der Stapelzaehler ist 1 oder der Zustand ist keine Funktion");
				}
			}
			else if (token == Dot) {
				if (lastOp && (lastOp->token == Variable || lastOp->token == BetweenBrackets || (lastOp->token == CallArgs && !isLastVarSet(lastOp)))) {
					...
					if (tokenType != Token_Variable) {
						SCRIPT_PARSER_ERR("Du greifst mit einem nicht-dynamischen Token auf ein Mitglied zu");
					}
					else {
					...
					SCRIPT_PARSER_ERR("Du greifst auf ein Mitglied einer Nicht-Variable zu");
				}
			}
					...
					if (lastOp && (lastOp->token == Variable || lastOp->token == BetweenBrackets || (lastOp->token == CallArgs && !isLastVarSet(lastOp) && !isLastVarCall(lastOp)))) {
						...
						*newBStack = *bstack;
						...
						if (!countTokens(bstack, EquationSeperator)) {
							...
							SCRIPT_PARSER_ERR("Prioritaets-Klammern koennen nur 1 Argument enthalten");
 						}
	...
	if (functionStack.count != 1 || stackHistoryHolder.count != 1) {
		SCRIPT_PARSER_ERR("Irgendwo scheint eine offene Klammer zu sein. Dateiende erreicht");
	}
