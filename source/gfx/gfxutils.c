...
void gfx_printTopInfo() {
    ...
    gfx_con_setpos(0, 0);
    gfx_printf("Tegraexplorer %d.%d.%d.%d | Akku: %d%% %c\n", LP_VER_MJ, LP_VER_MN, LP_VER_BF, LP_VER_SB, battery >> 8, ((current_charge_status) ? 129 : 32));
    RESETCOLOR;
}
...
MenuEntry_t YesNoEntries[] = {
    {.optionUnion = COLORTORGB(COLOR_YELLOW), .name = "Nein"},
    {.R = 255, .name = "Ja"}
};
